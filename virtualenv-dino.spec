...
python $RPM_BUILD_DIR/virtualenv-pylons/virtualenv.py $RPM_BUILD_ROOT/usr/local/%{virtualenv_name} 
rm $RPM_BUILD_ROOT/usr/local/%{virtualenv_name}/lib64
(cd $RPM_BUILD_ROOT/usr/local/%{virtualenv_name}; ln -s lib lib64)  # This is a small fix to make the path not absolute and point to RPM_BUILD_ROOT

%install

echo "PULLING Pylons"
ARGS="-f https://svn.metaweb.com/svn/thirdparty/trunk --allow-hosts svn.metaweb.com --always-unzip"
$RPM_BUILD_ROOT/usr/local/%{virtualenv_name}/bin/easy_install $ARGS %{package_list}

set +x
echo "FIXING virtualenv PATHS"
find -H $RPM_BUILD_ROOT/usr/local/%{virtualenv_name} -type f | while read filename; do    
