...
class DinoException(Exception):
    ...
    def format_trace(self, e=None):
        yield "\n"
        yield "TRACEBACK\n"
        for line in  self._format_exception(self, sys.exc_info()[2]):
            yield line

        for cause, cause_tb in self:
            for line in self._format_exception(cause, cause_tb):
                yield line


    def _format_exception(self, e, tb):
        yield "\n"
        yield "%s.%s: %s\n" % (e.__module__, e.__class__.__name__, str(e))
