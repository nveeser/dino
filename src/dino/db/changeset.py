...
class ChangeSetSession(sqlalchemy.orm.session.Session):
    ...
    def _clear_changeset(self):
        if self.opened_changeset:
            self.expunge(self.opened_changeset)
            self.opened_changeset = None
    ...
    class ChangeSetSessionExtension(sqlalchemy.orm.session.SessionExtension):
        ...
        def before_flush(self, session, flush_context, instances):
            ...
            found_revisioned_instance = False

            for instance in session.dirty:
                if hasattr(instance, 'changeset') and session.is_modified(instance):
                    self.assert_submitting(session)
                    instance.changeset = session._assert_changeset()

                    found_revisioned_instance = True

            for instance in session.deleted.union(session.new):
            ...
            if session.opened_changeset and found_revisioned_instance:
                session.opened_changeset.committed = datetime.datetime.utcnow()
                session.add(session.opened_changeset)
        ...
        def assert_submitting(self, session):
            if not session.submitting:
                raise RuntimeError("Don't flush revisioned Element(s) without a full commit. It's confusing")
