...
class DhcpGenerator(Generator):
    ...
    def find_next_server(self, addr):
        if '_next_server_map' not in vars(self):
            prefix = 'dhcp_next_server_'
            self._next_server_map = {}
            for (name, value) in self.settings.items():
                ...
                if name.startswith(prefix):
                    ...
                    self._next_server_map[subnet] = value

        if '_host_ip' not in vars(self):
            self._host_ip = socket.gethostbyname(socket.gethostname())

        for subnet in self._next_server_map.keys():
            if subnet.contains(addr):
                self.log.fine("network (%s) next-server %s", subnet, self._next_server_map[subnet])
                return self._next_server_map[subnet]

        self.log.warn("Assigning next-server: No subnet defined for address (using host IP): %s", addr)
        return str(self._host_ip)
    ...
    def query_subnets(self, session):
        ...
        for s in subnets:
            ...
            # Find the next-ip
            # 
            data['next-ip'] = self.find_next_server(s)
    ...
    def query_gentoo_hosts(self, session):
        ...
        for port in query:
            ...
            data['options'] = ""
            data['next-ip'] = self.settings.dhcp_gentoo_source
    ...
    def query_fedora_hosts(self, session):
        ...
        for port in query:
            ...
            # Find the next-ip
            # 
            data['next-ip'] = self.find_next_server(port.interface.address)
